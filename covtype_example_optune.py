...
def run_covtype(rank, world_size, experiment):
    ...
    def build_model(l_h, n_l):
        return LinearNet(54, 7, l_h, n_l)

    def train_and_eval(net, dataloader):
    ...
    def objective(trial, ):
        ...
        net = build_model(trial.suggest_int('l_h', 32, 1024), trial.suggest_int("n_l", 1, 4))
        # we recommend using the algorithm argument to determine the type of algorithm to be used

        accuracy = train_and_eval(net, dataloader)
