...
#--ipc=host -v /var/run/docker.sock:/var/run/docker.sock
DOCKER_RUN_COMMAND="docker run $port_mapping --ulimit memlock=-1 $gpu_option --shm-size=8g --memory=${backoff_memory_mb}m --ulimit stack=67108864 --restart unless-stopped $run_mode -v $HOME_DIR:$HOME_DIR -v /mnt/:/mnt/  -e INITIALS=${INITIALS} $MORE_DOCKER_ARGS --name $NAME --hostname $NAME $IMAGE $COMMAND"
